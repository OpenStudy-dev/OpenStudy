...
async def list_(
    ...
    return svc.list_lectures(course_code=course_code)
...
async def create(body: LectureCreate, _: bool = Depends(require_auth)) -> Lecture:
    return svc.create_lecture(body)
...
async def patch(
    ...
    return svc.update_lecture(lecture_id, body)
...
async def attended(
    ...
    return svc.mark_attended(lecture_id, attended=attended)
